...
import { isPomodoro, getPomodoro, mmss, lighten, glowColor, cssVars } from '../helpers';
...
/** 舞台底部快捷键提示：可运行三类共用同一段文案，改提示只改一处 */
const HINTS_RUN = 'Space 暂停/继续 · N 打点 · 双击侧栏行进入台钟';
const HINTS_POMO = 'Space 开始/暂停 · N 跳过 · 双击侧栏行进入台钟';
const HINTS_DATE = '双击侧栏行进入台钟 · Esc 返回';
...
function MilestonePanel({ timerId, active }: { timerId: string; active: boolean }) {
  ...
  // 内联输入替代 window.prompt：原生 prompt 阻塞整个主进程、样式与深色主题完全割裂
  function submit() {
    const note = draft.trim();
    if (!note) return;
    window.timemark.addMilestone(timerId, note).then(() => { setDraft(''); setComposing(false); reload(); }).catch(() => {});
  }
  return (
      ...
      <div className="seg-head">
        ...
        {active && (composing ? (
          <span className="ms-add">
            ...
            />
            <button style={addBtnStyle} onClick={submit}>标记</button>
          </span>
        ) : (
          <button style={addBtnStyle} onClick={() => setComposing(true)}>+ 标记此刻</button>
        ))}
      </div>
...
export default function FocusStage({ timer: t, actions }: Props) {
  ...
  if (isPomodoro(t)) {
    ...
    // 空闲态画空环：p=1 满环 + 「剩余 100%」会被误读成「刚开始就快结束」
    const p = active ? (phaseMs > 0 ? rem / phaseMs : 0) : 0;
    ...
    return (
      <div className="stage" style={tcStyle}>
        <FocusRing
          ...
          center={
            <>
              <div className="ring-num">{mmss(rem)}</div>
              <div className="ring-sub">{active ? `${isWork ? '剩余' : '休息剩余'} ${Math.round(p * 100)}%` : '待开始'}</div>
            </>
          }
        />
        ...
        <div className="stage-hints">{HINTS_POMO}</div>
      </div>
    );
  }
  ...
  if (t.type === 'PRECISE_COUNTDOWN') {
    ...
    return (
      <div className="stage" style={tcStyle}>
        ...
        <div className="stage-hints">{HINTS_RUN}</div>
      </div>
    );
  }
    ...
    return (
      <div className="stage" style={tcStyle}>
        <div className="stage-num-block">
          ...
          <div className="stage-desc">
            {active
              ? <>本段 {formatElapsed(curLap)} · N 键记录一段（lap）</>
              : t.runState === 'paused'
                ? <>已暂停 · 累计 {formatElapsed(elapsed)}</>
                : <>从 0 开始累计 · 支持分段与暂停</>}
          </div>
        </div>
          ...
          <div className="seg-card">
            ...
            {segs.map((ms, i) => (
              <div className="seg-row" key={i} style={cssVars({ '--i': i })}>
                <span className="idx">{i + 1}.</span>
                <span className="seg-name">第 {i + 1} 段</span>
                <span className="seg-val">{formatElapsed(ms)}</span>
              </div>
            ))}
          </div>
        )}
  ...
  return (
    <div className="stage" style={tcStyle}>
      ...
      <div className="stage-hints">{HINTS_DATE}</div>
    </div>
  );
}
